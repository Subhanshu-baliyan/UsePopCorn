              ...
              {!isWatched ? (
                <>
                  <StarRating
                    maxStar={10}
                    size={24}
                    onStateChange={setUserRating}
                  />
                  {userRating > 0 && (
                    <button className="btn-add" onClick={handleNewWatched}>
                      + Add this to list
                    </button>
                  )}
                </>
              ) : (
                <p>You already rated it {ratedNum} ⭐️</p>
              )}
            </div>
            <p>
              <em>{plot}</em>
...
function WatchedList({ watched, onDeleteWatch }) {
  return (
    <ul className="list">
      {watched.map((movie) => (
        <WatchedMovie
